use crate::{domain::models::todo::Todo, infrastructure::data::db_context::db_context::DB};
...
impl TodoRepository {
    ...
    pub async fn create_todo(&self, content: Todo) -> Result<Vec<Todo>> {
        let record = DB.create(&self.table).content(content).await?;
        if let Some(todo) = record {
            Ok(vec![todo])
        } else {
            Err(anyhow!("Failed to create todo"))
        }
    }
