...
pub fn utils_lm_attribute(llmPrompt: RequestBody) -> String {
    ...
    if (modelrequest.is_empty() || modelrequest == "") {
        ...
        error!("{} # {}",log.clone(),"RAG EMBED: FILL MODEL Prompt Command REQ ".to_string());
        process::exit(0x0100);
        // ExitCode::FAILURE
        // Err("".to_string())
    }
    ...
    info!("{} embed LLM: nameModel {}",log.clone(),nameModel);
    info!("{} embed LLM: versionModel {}",log.clone(),versionModel);
